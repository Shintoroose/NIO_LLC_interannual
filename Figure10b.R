...
#---------------------------------EQuatorial Wind------------------------------------------------
Year<-seq(1979,2020)
system("cdo -fldmean -sellonlatbox,83,95,-2.5,2.5 -yearmean -selmon,10/12 -sellevel,200 ./DATASETS/ERA5/Monthly/UV_850_200_1979_2020.nc ./junk/eqwin.nc")
...
oni <- readLines("./ShintoRoose_Laptop/Documents/indices/ONI/Oceanic_Nino_Index_ONI_ERSST_v5_SST.txt")
...
QWIN_VWS<-ggplot(df, aes(x=Eqwin_norm, y=VWS_norm, color=ENSO)) +
  ...
  scale_x_continuous(name="Normalized anomaly of equatorial wind (200 hPa)",limits = c(-3, 3), breaks = seq(-3,3,1)) +
  #scale_y_continuous(name="Normalized anomaly of vertical wind shear",limits = c(-3, 3), breaks = seq(-3,3,1)) +
  #scale_x_continuous(name="Normalized anomaly of equatorial wind (850 hPa)",limits = c(-1, 1), breaks = seq(-1,1,0.5)) +
  ...
  geom_label(data=df, aes(x=2.5, y=3, label = "r=0.86"),size=4,color = "royalblue4", family="Times New Roman") +
...
#----------------------------------------------------------
QWIN_VWS
ggsave("./Figures/EQWIN200_VWS_5N10N_new.png", width = 15, height = 15, units = "cm", dpi = 300)
