...
dmi <- readLines("./ShintoRoose_Laptop/Documents/indices/DMI/dmi.had.long.data.txt")
...
#----------------------------------------------------------
cor.test(df$Eqwin_norm,df$AbsVo_norm)
theme_set(theme_gray())
QWIN_ETA<-ggplot(df, aes(x=Eqwin_norm, y=AbsVo_norm, color=ENSO)) +
  geom_point() + 
  ...
  scale_y_continuous(name="Normalized anomaly of absolute vorticity (850 hPa)",limits = c(-3, 3), breaks = seq(-3,3,1)) +
  ...
  geom_label(data=df, aes(x=2.5, y=3, label = "r=0.91"),size=4,color = "royalblue4", family="Times New Roman") +
...
QWIN_ETA
#----------------------------------------------------------
ggsave("./Figures/EQWIN_ETA_5N10N_new.png", width = 15, height = 15, units = "cm", dpi = 300)
